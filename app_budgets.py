...
# Function to retrieve all budgets for a given user based on their user ID
...
def getTotalBudgetedByYear(userID, year=None):
    ...
    # query = "SELECT SUM(amount) AS amount FROM budgets WHERE user_id = %s AND EXTRACT(YEAR FROM year) = %s"
    # params = {"usersID": userID, "year": year}
...
def getUpdatableBudget(budget, userID):
    ...
    categories = app_categories.getSpendCategories(userID)
