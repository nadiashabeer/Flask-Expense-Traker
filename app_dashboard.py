...
def getTotalSpend_Year(user_id):
    # Function to retrieve the total spend of a user for the current calendar year
    ...
    total_spend_year = convertSQLToDict(results)

    return total_spend_year[0]['expenses_year']


# Get and return the users total spend for the current month
def getTotalSpend_Month(user_id):
    results = db.execute(
...
def getTotalSpend_Week(user_id):
...
def getLastFiveExpenses(user_id):
    results = db.execute(
        "SELECT description, category, expenseDate, payer, amount FROM expenses WHERE user_id = :usersID ORDER BY id DESC LIMIT 5", {"usersID": user_id}).fetchall()
...
def getBudgets(user_id, year=None):
    budgets = []
    ...
    budgets_query = app_budgets.getBudgets(user_id)
    ...
    if budgets_query and year in budgets_query:
        for record in budgets_query[year]:
            ...
            results = db.execute(
                ...
                {"usersID": user_id, "year": year, "budgetID": budgetID}).fetchall()
...
def getWeeklySpending(weekNames, user_id):
    weeklySpending = []
    ...
    for name in weekNames:
        ...
        results = db.execute(
            ...
            {"usersID": user_id, "weekName": str(name["endofweek"])}).fetchall()
...
def getMonthlySpending(user_id, year=None):
    spending_month = []
    ...
    results = db.execute(
        ...
        {"usersID": user_id, "year": year}).fetchall()
...
def getSpendingTrends(user_id, year=None):
