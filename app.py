...
def index():
    ...
    if request.method == "GET":
        ...
        categories = app_categories.getSpendCategories(session["user_id"])

        # Get the users payers (for quick expense modal)
        payers = app_account.getPayers(session["user_id"])

        # Get todays date (for quick expense modal)
        date = datetime.today().strftime('%Y-%m-%d')

        # Get the users income
        income = app_account.getIncome(session["user_id"])

        # Get current years total expenses for the user
        expenses_year = app_dashboard.getTotalSpend_Year(session["user_id"])

        # Get current months total expenses for the user
        expenses_month = app_dashboard.getTotalSpend_Month(
            session["user_id"])

        # Get current week total expenses for the user
        expenses_week = app_dashboard.getTotalSpend_Week(session["user_id"])

        # Get last 5 expenses for the user
        expenses_last5 = app_dashboard.getLastFiveExpenses(
            session["user_id"])

        # Get every budgets spent/remaining for the user
        budgets = app_dashboard.getBudgets(session["user_id"])

        # Get weekly spending for the user
        weeks = app_dashboard.getLastFourWeekNames()
        spending_week = app_dashboard.getWeeklySpending(
            weeks, session["user_id"])

        # Get monthly spending for the user (for the current year)
        spending_month = app_dashboard.getMonthlySpending(
            session["user_id"])

        # Get spending trends for the user
        spending_trends = app_dashboard.getSpendingTrends(
            session["user_id"])

        # Get payer spending for the user
        payersChart = app_reports.generatePayersReport(session["user_id"])
    ...
    # User reached route via POST
        ...
        expenses = app_expenses.add_expenses(formData, session["user_id"])
...
def expensehistory():
    ...
    if request.method == "GET":
        ...
        history = app_expenses.getHistory(session["user_id"])
    ...
    # User reached route via POST
        ...
        oldExpense = app_expenses.getExpense(
        ...
        if userHasSelected_deleteExpense == True:
            ...
            deleted = app_expenses.deleteExpense(
            ...
            history = app_expenses.getHistory(session["user_id"])
            categories = app_categories.getSpendCategories(
                session["user_id"])
            payers = app_account.getPayers(session["user_id"])
        ...
        # Update the existing expense record
            ...
            expensed = app_expenses.updateExpense(
...
def budgets(year=None):
    ...
    if request.method == "GET":
        ...
        budgets = app_budgets.getBudgets(session["user_id"])

        # Get the users total budgeted amount
        budgeted = app_budgets.getTotalBudgetedByYear(
    ...
    # User reached route via POST
        ...
        # Delete the budget
        deletedBudgetName = app_budgets.deleteBudget(
...
def createbudget():
    ...
    if request.method == "POST":
        ...
        budgetDict = app_budgets.generateBudgetFromForm(formData)
        ...
        if "apology" in budgetDict:
            ...
            budget = app_budgets.createBudget(
        ...
        # Get the users total budgeted amount
        budgeted = app_budgets.getTotalBudgetedByYear(session["user_id"])
...
def updatebudget(urlvar_budgetname):
    ...
    if request.method == "POST":
        ...
        if "apology" in budgetDict:
            ...
            budget = app_budgets.updateBudget(
    ...
    # User reached route via GET
        ...
        budgetID = app_budgets.getBudgetID(
            urlvar_budgetname, session["user_id"])
        if budgetID is None:
            return apology("'" + urlvar_budgetname + "' budget does not exist")
        else:
            budget = app_budgets.getBudgetByID(budgetID, session["user_id"])
        ...
        budget = app_budgets.getUpdatableBudget(budget, session["user_id"])
...
def categories():
    ...
    if request.method == "POST":
        ...
        if userHasSelected_newCategory:
            ...
            categoryCount = len(
                app_categories.getSpendCategories(session["user_id"]))
            if categoryCount >= 30:
            ...
            categoryID = app_categories.getCategoryID(newCategoryName)
            ...
            if categoryID:
                ...
                existingID = app_categories.getCategoryID(
                ...
                # Add the category to the users account
                    ...
                    app_categories.addCategory_User(
            ...
            # Category does not exist in the DB already - create a new category and then add it to the users account
                ...
                newCategoryID = app_categories.addCategory_DB(
                    newCategoryName)
        ...
        if userHasSelected_renameCategory:
            ...
            oldCategoryID = app_categories.getCategoryID(oldCategoryName)
            ...
            # Check to see if the *new* name already exists in the database (None == does not exist)
            newCategoryID = app_categories.getCategoryID(newCategoryName)
            ...
            if newCategoryID:
                ...
                newCategoryNameFromDB = app_categories.getSpendCategoryName(
                    newCategoryID)

                # Rename the category
                app_categories.renameCategory(
        ...
        if userHasSelected_deleteCategory:
            ...
            categoryID = app_categories.getCategoryID(deleteName)
            ...
            # Delete the category
            app_categories.deleteCategory(categoryID, session["user_id"])
    ...
    # User reached route via GET
        ...
        categoryBudgets = app_categories.getBudgetsSpendCategories(
            session["user_id"])

        # Generate a single data structure for storing all categories and their associated budgets
        categoriesWithBudgets = app_categories.generateSpendCategoriesWithBudgets(
...
def budgetsreport(year=None):
    ...
    budgets = app_reports.generateBudgetsReport(session["user_id"], year)
...
def monthlyreport(year=None):
    ...
    monthlySpending = app_reports.generateMonthlyReport(
        session["user_id"], year)
...
def spendingreport(year=None):
    ...
    spendingReport = app_reports.generateSpendingTrendsReport(
        session["user_id"], year)
...
def payersreport(year=None):
    ...
    payersReport = app_reports.generatePayersReport(
        session["user_id"], year)
...
def updateaccount():
    ...
    if request.method == "POST":
        ...
        if userHasSelected_updateIncome:
            ...
            updatedIncome = app_account.updateIncome(
        ...
        if userHasSelected_addPayer:
            ...
            # Add the payer
            newPayer = app_account.addPayer(newName, session["user_id"])
        ...
        if userHasSelected_renamePayer:
            ...
            # Rename the payer
            renamedPayer = app_account.renamePayer(
        ...
        if userHasSelected_deletePayer:
            ...
            # Delete the payer
            deletedPayer = app_account.deletePayer(name, session["user_id"])
        ...
        if userHasSelected_updatePassword:
            ...
            updatedPassword = app_account.updatePassword(request.form.get(
        ...
        user = app_account.getAllUserInfo(session["user_id"])
